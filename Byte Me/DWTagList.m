...
- (void)setup {
    ...
    self.automaticResize = DW_DEFAULT_AUTOMATIC_RESIZE;
    self.highlightedBackgroundColor = DW_HIGHLIGHTED_BACKGROUND_COLOR;
    self.font = [UIFont fontWithName:@"Quicksand-Bold" size:DW_FONT_SIZE_DEFAULT];//[UIFont systemFontOfSize:DW_FONT_SIZE_DEFAULT];
    self.labelMargin = DW_LABEL_MARGIN_DEFAULT;
    self.bottomMargin = DW_BOTTOM_MARGIN_DEFAULT;
    self.horizontalPadding = DW_HORIZONTAL_PADDING_DEFAULT;
    self.verticalPadding = DW_VERTICAL_PADDING_DEFAULT;
    self.cornerRadius = DW_CORNER_RADIUS;
    self.borderColor = DW_BORDER_COLOR;
    self.borderWidth = DW_BORDER_WIDTH;
    self.textColor = DW_TEXT_COLOR;
    self.textShadowColor = DW_TEXT_SHADOW_COLOR;
    self.textShadowOffset = DW_TEXT_SHADOW_OFFSET;
    self.showTagMenu = DW_DEFAULT_SHOW_TAG_MENU;
}
...
- (void)display
{
    ...
    for (NSDictionary * dic in textArray) {
        ...
        NSString *text = [@"#" stringByAppendingString:path];
...
- (void)touchUpInside:(id)sender
{
    ...
    if ([self.tagDelegate respondsToSelector:@selector(selectedTag:tagIndex:path:)])
    {
        [self.tagDelegate selectedTag:tagView.label.text tagIndex:tagView.tag path:tagView.path];
    }
...
{
    return !lblBackgroundColor ? DW_BACKGROUND_COLOR : lblBackgroundColor;
}
...
- (id)init
{
    ...
    if (self) {
        ...
        [_label setTextColor:DW_TEXT_COLOR];
        [_label setShadowColor:DW_TEXT_SHADOW_COLOR];
        [_label setShadowOffset:DW_TEXT_SHADOW_OFFSET];
        ...
        [self.layer setCornerRadius:DW_CORNER_RADIUS];
        [self.layer setBorderColor:DW_BORDER_COLOR.CGColor];
        [self.layer setBorderWidth:DW_BORDER_WIDTH];
    }
    return self;
}

- (void)updateWithString:(id)text font:(UIFont*)font constrainedToWidth:(CGFloat)maxWidth padding:(CGSize)padding minimumWidth:(CGFloat)minimumWidth path:(NSString*)path
{
    ...
    textSize.height += DW_VERTICAL_PADDING_DEFAULT + padding.height*2;
