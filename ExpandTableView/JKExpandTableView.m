...
- (void) initialize {
    ...
    self.separatorStyle = UITableViewCellSeparatorStyleNone;//UITableViewCellSeparatorStyleSingleLine;
...
- (void) expandForParentAtRow: (NSInteger) row {
    ...
    //[self collapseAllExceptRow:row];

}

-(void) collapseAllExceptRow:(NSInteger) row
{
    for(int i =0; i < [self.expansionStates count]; i++)
    {
        if((int)row != i)
        {
            BOOL e = [[self.expansionStates objectAtIndex:i] boolValue];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (isExpansionCell) {
        ...
        int  sectionId = [self.dataSourceDelegate idForParentCellAtIndex:parentIndex];
        [cell setSectionId:sectionId];
        ...
        if ([self.dataSourceDelegate respondsToSelector:@selector(iconLeftForCellAtIndex:)]) {
            UIImage *icon = [self.dataSourceDelegate iconLeftForCellAtIndex:parentIndex];
            [[cell iconImageLeft] setImage:icon];
        }
        
        if ([self.dataSourceDelegate respondsToSelector:@selector(iconLeftColourForCellAtIndex:)]) {
            UIColor *color = [self.dataSourceDelegate iconLeftColourForCellAtIndex:parentIndex];
            [[cell iconImageLeft] setTintColor:color];
        }
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([selectedCell isKindOfClass:[JKParentTableViewCell class]]) {
        ...
        NSInteger children = [self numberOfChildrenUnderParentIndex: [pCell parentIndex]];
...
- (BOOL) isHighlightedForChildIndex:(NSInteger)childIndex underParentIndex:(NSInteger)parentIndex {
    return [self.dataSourceDelegate shouldHighlightCellAtChildIndex:childIndex withinParentCellIndex:parentIndex];
}
...
- (int ) idForChildIndex:(NSInteger)childIndex underParentIndex:(NSInteger)parentIndex {
    return [self.dataSourceDelegate idForCellAtChildIndex:childIndex withinParentCellIndex:parentIndex];
}
...
- (UIColor *) iconColourForChildIndex:(NSInteger)childIndex underParentIndex:(NSInteger)parentIndex {
    if ([self.dataSourceDelegate respondsToSelector:@selector(iconColourForCellAtChildIndex:withinParentCellIndex:)]) {
        return [self.dataSourceDelegate iconColourForCellAtChildIndex:childIndex withinParentCellIndex:parentIndex];
    } else {
        return nil;
    }
}
